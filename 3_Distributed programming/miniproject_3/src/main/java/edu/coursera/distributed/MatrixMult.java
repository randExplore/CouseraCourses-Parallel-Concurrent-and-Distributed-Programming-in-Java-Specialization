...
public class MatrixMult {
    ...
    public static void parallelMatrixMultiply(Matrix a, Matrix b, Matrix c,
        ...
        final int curRank = mpi.MPI_Comm_rank(mpi.MPI_COMM_WORLD);
        final int clusterSize = mpi.MPI_Comm_size(mpi.MPI_COMM_WORLD);
        final int numRows = c.getNRows();
        final int chunkSize = (numRows + clusterSize - 1) / clusterSize;
        final int startRow = curRank * chunkSize;
        int endRow = (curRank + 1) * chunkSize;
        ...
        if (curRank == 0) {
            MPI.MPI_Request[] requests = new MPI.MPI_Request[clusterSize - 1];
            for (int i= 1; i<clusterSize; i++) {
                final int rankStartRow = i * chunkSize;
                int rankEndRow = (i + 1) * chunkSize;
                if (rankEndRow > numRows) rankEndRow = numRows;
                final int offset = rankStartRow * c.getNCols();
                final int count = (rankEndRow - rankStartRow) * c.getNCols();
                requests[i-1] = mpi.MPI_Irecv(c.getValues(), offset, count, i, i, mpi.MPI_COMM_WORLD);
            }
            mpi.MPI_Waitall(requests);
        } else {
            mpi.MPI_Isend(c.getValues(), startRow * c.getNCols(),
                    (endRow - startRow) * c.getNCols(), 0, curRank, mpi.MPI_COMM_WORLD);
        }
    }
}
